  ...
  <div class="space-y-2 relative" ref="dropdownContainer">
    ...
    <div v-else-if="!isNewCustomer" class="relative">
      ...
      <input 
        ...
        class="w-full h-11 pl-11 pr-20 rounded-xl text-sm font-medium bg-muted/50 border-2 border-divider focus:outline-none focus:border-indigo-400/50 transition-all" 
      />
      <button 
        type="button"
    ...
    <div v-if="isNewCustomer" class="mt-2 p-5 rounded-xl bg-muted/20 border-2 border-border/50 space-y-3 relative group transition-all">
      ...
      <input v-model="newCustomerForm.name" placeholder="Full Name *" class="w-full h-11 px-4 rounded-2xl text-sm font-medium bg-muted/50 border-2 border-divider focus:outline-none focus:border-indigo-400/50 transition-all" />
      <input v-model="newCustomerForm.email" placeholder="Email Address" type="email" class="w-full h-11 px-4 rounded-2xl text-sm font-medium bg-muted/50 border-2 border-divider focus:outline-none focus:border-indigo-400/50 transition-all" />
      <input v-model="newCustomerForm.phone" placeholder="Phone Number" type="tel" class="w-full h-11 px-4 rounded-2xl text-sm font-medium bg-muted/50 border-2 border-divider focus:outline-none focus:border-indigo-400/50 transition-all" />
      
      <button 
        type="button"
        class="w-full h-11 mt-2 rounded-2xl text-sm font-bold text-white flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
