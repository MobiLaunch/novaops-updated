<template>
  <div class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style="background: hsl(var(--background))">
    <div class="absolute inset-0 pointer-events-none overflow-hidden">
      <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] rounded-full opacity-10 blur-3xl" style="background: radial-gradient(circle, #5b5ef4, transparent)" />
    </div>
    <div class="relative w-full max-w-lg flex flex-col gap-7" style="animation: setupEnter 0.5s cubic-bezier(0.34,1.3,0.64,1) both">
      <div class="text-center flex flex-col items-center gap-4">
        <div class="w-20 h-20 rounded-2xl flex items-center justify-center shadow-2xl" style="background: linear-gradient(135deg, #5b5ef4 0%, #8b5cf6 100%); box-shadow: 0 8px 40px #5b5ef460">
      ...
      <div class="rounded-2xl bg-card p-8 flex flex-col gap-5 shadow-xl" style="border: 1px solid hsl(var(--border)/0.7)">
        <div class="space-y-2">
        ...
        <div class="space-y-2">
          ...
          <div class="flex gap-2 flex-wrap">
            <button v-for="c in currencies" :key="c.value" class="px-4 py-2.5 rounded-full text-xs font-bold transition-all hover:scale-105 active:scale-95"
              :style="profile.currency === c.value ? 'background: #5b5ef424; color: #5b5ef4; outline: 2px solid #5b5ef450; outline-offset: 0' : 'background: hsl(var(--muted)/0.5); color: hsl(var(--muted-foreground))'"
              @click="profile.currency = c.value">{{ c.label }}</button>
          </div>
        </div>

        <!-- Error banner -->
        <div v-if="errorMsg" class="flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm font-semibold" style="background: #ef444412; color: #ef4444; outline: 1.5px solid #ef444428; outline-offset: 0">
...
.m3-input:focus { border-color: #5b5ef4; box-shadow: 0 0 0 3px #5b5ef418; }
