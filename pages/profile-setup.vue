  ...
  <div class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style="background: hsl(var(--background))">
    ...
    <div class="relative w-full max-w-lg flex flex-col gap-7" style="animation: setupEnter 0.5s cubic-bezier(0.34,1.3,0.64,1) both">
      <div class="text-center flex flex-col items-center gap-4">
        <div class="w-20 h-20 rounded-3xl flex items-center justify-center shadow-2xl" style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); box-shadow: 0 8px 40px #6366f160">
      ...
      <div class="rounded-3xl bg-content1 p-8 flex flex-col gap-5 shadow-xl">
        <div class="space-y-2">
          <label class="hui-label">Business Name *</label>
          <input v-model="profile.businessName" placeholder="NovaOps Repair Shop" class="hui-input" />
        </div>
        <div class="space-y-2">
          <label class="hui-label">Business Email *</label>
          <input v-model="profile.email" type="email" placeholder="contact@yourshop.com" class="hui-input" />
        </div>
        <div class="space-y-2">
          <label class="hui-label">Phone Number</label>
          <input v-model="profile.phone" type="tel" placeholder="(555) 123-4567" class="hui-input" />
        </div>
        <div class="space-y-2">
          <label class="hui-label">Business Address</label>
          <textarea v-model="profile.address" placeholder="123 Main St, City, State 12345" :rows="2" class="hui-input resize-none" style="height: auto; padding-top: 12px; padding-bottom: 12px;" />
        </div>
        <div class="space-y-2">
        ...
        <!-- Error banner -->
        <div v-if="errorMsg" class="flex items-center gap-2.5 px-4 py-3 rounded-2xl text-sm font-semibold" style="background: #ef444412; color: #ef4444; outline: 1.5px solid #ef444428; outline-offset: 0">
          <AlertCircle class="w-4 h-4 flex-shrink-0" />
          {{ errorMsg }}
        </div>

        <button class="hui-btn hui-btn-solid-primary hui-btn-lg w-full h-14 rounded-full text-sm font-black text-white shadow-lg flex items-center justify-center gap-2.5 disabled:opacity-60"
...
.hui-input:focus { border-color: #6366f1; box-shadow: 0 0 0 3px #6366f118; }
