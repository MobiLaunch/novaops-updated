  ...
  <div class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style="background: hsl(var(--background))">
    ...
    <div class="relative w-full max-w-sm flex flex-col gap-7" style="animation: registerEnter 0.5s cubic-bezier(0.34,1.3,0.64,1) both">
      ...
      <div class="rounded-[32px] bg-card p-7 flex flex-col gap-5 shadow-xl" style="outline: 2px solid hsl(var(--border)/0.6); outline-offset: 0">
        ...
        <button class="m3-jelly-btn w-full h-14 rounded-full text-sm font-black text-white shadow-lg flex items-center justify-center gap-2.5 disabled:opacity-60" style="background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); box-shadow: 0 4px 20px #8b5cf650" :disabled="loading" @click="handleRegister">
        ...
        <div class="relative flex items-center gap-3">
          ...
          <span class="text-xs text-muted-foreground font-semibold">Already have one?</span>
          <div class="flex-1 h-px" style="background: hsl(var(--border)/0.6)" />
        </div>
...
const handleRegister = async () => {
  ...
  try {
    const { data, error: authError } = await ($supabase as any).auth.signUp({ email: email.value, password: password.value })
...
.m3-tonal-btn { transition: transform 0.4s cubic-bezier(0.34,1.56,0.64,1); }
